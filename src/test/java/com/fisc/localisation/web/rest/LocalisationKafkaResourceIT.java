package com.fisc.localisation.web.rest;

import com.fisc.localisation.config.KafkaProperties;
...
class LocalisationKafkaResourceIT {
    ...
    void setup() {
        ...
        LocalisationKafkaResource kafkaResource = new LocalisationKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/localisation-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/localisation-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
